...
window.FX_DEFAULTS = {
  ...
  art: {
    // маркерный стиль карты и «живая» анимация (см. public/js/art.js)
    enabled: true,   // маркерные штрихи, бумага, тени
    motion: true,    // качка судов, ряби, рыба, флаги (при prefers-reduced-motion — выкл. сама)
    grain: 1,        // зерно бумаги 0..1
    waves: 1         // плотность рябей 0..2
  },
  gold: {
    // Интервал между ВСЕМИ всплывающими над кораблём надписями: −урон, +лечение, +золото, +монеты.
    // Они показываются строго по очереди с этим шагом — иначе наезжают друг на друга (залп даёт
    // несколько попаданий разом, а за потоплением сразу идут награды).
    gap: 420,

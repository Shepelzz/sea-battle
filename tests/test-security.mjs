...
// Отказ по потолку — снимок состояния, а не приговор: закрыл лишнюю партию — сообщение
// обязано исчезнуть само, без перезагрузки страницы.
{
  const home = readFileSync('public/js/home.js', 'utf8');
  yes('гасятся все три редактора сразу',
    /EDITOR_ERRORS = \['#createError', '#hotseatError', '#botError'\]/.test(home));
  yes('ошибка гаснет при смене экрана', /function showMode\(mode\) \{\s*clearEditorErrors\(\);/.test(home));
  yes('ошибка гаснет после закрытия партии',
    /function finishGame\([\s\S]{0,300}clearEditorErrors\(\)/.test(home));
  yes('ошибка гаснет на новой попытке во всех трёх редакторах',
    (home.match(/clearEditorErrors\(\);/g) || []).length >= 5);
}

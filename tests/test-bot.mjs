...
function setup() {
  ...
  const g = createGame('t', { maxPlayers: 2, turnTimer: 0, seed: 4242, mapScale: 1, fixedCorners: true });
  addPlayer(g, 'p0', 'Я');
  addPlayer(g, 'p1', 'Враг');
  startGame(g, 'p0');
  g.ships = [];                                   // чистый стол
...

{
  ...
  for (let seed = 1; seed <= 6; seed++) {
    const g = createGame('op' + seed, { maxPlayers: 2, turnTimer: 0, seed, mapScale: 1, fixedCorners: true });
...
// на замере в 80 партий «Адмирал» бьёт «Юнгу» в 90% случаев.
{
  ...
  for (let n = 0; n < 36; n++) {
    const g = createGame('lad' + n, { maxPlayers: 2, turnTimer: 0, seed: 1 + (n >> 1), mapScale: 1, fixedCorners: true });
    g.config.multiMove = true; g.config.botGame = true; g.config.fog = true;
    const hardIdx = n % 2;
    addPlayer(g, 'p0', 'A'); addPlayer(g, 'p1', 'B');
...
// === ⛵ СТРОЙ: бот перегоняет эскадру пачкой, но не в бою ===
// Конвой стоит один манёвр на весь строй, так что подкрепление за тридевять земель бот обязан
// гнать вместе. И обязан НЕ собирать строй в боевом контакте — это правило игры, не совет.
{
  // Флот заметно БОЛЬШЕ бюджета хода: только тогда строй и выгоден — три судна за один манёвр,
  // а остальными ход продолжается. (Ровно три корабля на три манёвра строем вести незачем:
  // поодиночке каждый пройдёт свою полную дальность, и бот честно так и делает.)
  const far = () => {
    const g = setup();
    g.config.multiMove = true;
    g.wind = { ang: 0, str: 0 };
    for (const [x, y] of [[300, 300], [345, 325], [310, 355], [355, 380], [265, 330], [390, 340], [280, 395]])
      put(g, 0, 'brig', x, y);
    put(g, 1, 'shkhuna', 1400, 1000);     // враг на том конце карты — надо перегонять флот
    return g;
  };
  const r = majority(far, a => a.type === 'convoy');
  check('строй: бот гонит эскадру пачкой через карту', r.ok, `${r.hits}/${r.tries} (${r.seen})`);

  // и этот строй должен приниматься сервером (бот не предлагает заведомо невалидных ходов),
  // вести НЕСКОЛЬКО судов и стоить один манёвр — ход после него продолжается
  let acc = 0, tries = 0, cheap = 0, crews = 0;
  for (let i = 0; i < 7; i++) {
    const gg = far();
    const a = chooseBotAction(gg, 0, 'hard');
    if (a.type !== 'convoy') continue;
    tries++;
    crews += 1 + (a.ships || []).length;
    if (!applyAction(gg, 'p0', a).ok) continue;
    acc++;
    if (gg.turn.moves === 1 && gg.turn.idx === 0) cheap++;
  }
  check('строй: предложенный ботом строй сервер принимает', tries > 0 && acc === tries, `${acc}/${tries}`);
  check('строй: строй съедает один манёвр, ход продолжается', tries > 0 && cheap === tries, `${cheap}/${tries}`);
  check('строй: ведёт больше одного судна', tries > 0 && crews / tries > 1.5, `в среднем ${(crews / Math.max(1, tries)).toFixed(1)} судна`);
}
{
  const hot = () => {
    const g = setup();
    ...
    for (const [x, y] of [[300, 300], [345, 325], [310, 355], [355, 380], [265, 330], [390, 340], [280, 395]])
      put(g, 0, 'brig', x, y);
    put(g, 1, 'fregat', 470, 340);        // в зоне выстрела — строй собирать нельзя
    return g;
  };
  const r = majority(hot, a => a.type !== 'convoy');

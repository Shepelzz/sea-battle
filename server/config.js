...
export const RT = {
  ...
  // ЭКОНОМИКА РЕАЛТАЙМА (пересмотр 2026-07-08): «раунд» ≈ MOVE_SECONDS (15с — корабль проходит
  // свою дальность хода). Раньше деньги капали каждые 8с — почти 2 «хода» дохода за один «ход»
  // движения, экономика печатала вдвое: бесконечные волны флотов. Теперь доход = раз в «раунд».
  INCOME_MS: 15000,         // доход порта раз в столько мс (вместо «в начале хода»)
  FISH_MS: 15000,           // пассивная рыбалка раз в столько мс
  // лут с обломков в реалтайме скромнее пошагового (0.5): при 50% взаимный размен флотов возвращал
  // каждой стороне половину стоимости врага — война окупалась, партии не сходились
  WRECK_LOOT_FRAC: 0.3,
...
export const RT_OUTPOST_MS = 15000;     // реалтайм: период дохода/ремонта/выстрела форта (раз в «раунд» ≈ MOVE_SECONDS)

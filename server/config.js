...
// Корабль бьёт залпом в ВЫБРАННЫЙ борт (наводя как при движении) — ВЕСЬ борт стреляет по КАЖДОМУ вражескому
// судну с этой стороны в радиусе. Урон по цели = (урон судна st.dmg) × angle(положение) × falloff(дистанция):
//   • angle — где враг в секторе: 1.0 на ПЕРПЕНДИКУЛЯРЕ к центру борта (макс, как у мортиры) → BROADSIDE_SIDE_MIN по краям сектора;
//   • falloff — ближе к борту больнее (1.0 в упор → BROADSIDE_FALLOFF_MIN на краю радиуса огня).
// Т.е. идеально наведённый борт в упор = полный урон судна (st.dmg) по КАЖДОЙ цели — против мортиры (которая по судам бьёт вполовину) залп выгоден.
// Сектор борта BROADSIDE_HALF_ARC (~46° в каждую сторону) — нос/корма не задеваются (для прямого выстрела есть мортира).
// Дальность = fireRange. За борт можно стрельнуть раз в ход; второй борт — добор (см. game.js).
export const BROADSIDE_CANNONS = { shkhuna: 2, brig: 3, fregat: 4, linkor: 4, carrier: 6 }; // пушек на борт по классам (для вида: трассеры/таблица флота)
...
export const BROADSIDE_SIDE_MIN = 0.2;       // доля урона на КРАЯХ сектора борта (на перпендикуляре к центру = 1.0); 0.2 = край урезан на 80%

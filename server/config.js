...
export const RT = {
  ...
  // ЭКОНОМИКА РЕАЛТАЙМА (пересмотр 2026-07-08): изначально деньги капали каждые 8с — вдвое чаще
  // «раунда» (MOVE_SECONDS 15с), экономика печатала и партии не сходились (бесконечные волны флотов).
  // Чистый раунд-темп (15с) оказался ДУШНЫМ: боты сидели без денег, перевес предрешал катку.
  // Компромисс по фидбеку: 12с = раунд-темп + 30% срезанного возвращено.
  INCOME_MS: 12000,         // доход порта раз в столько мс (вместо «в начале хода»)
  FISH_MS: 12000,           // пассивная рыбалка раз в столько мс
  // лут с обломков в реалтайме скромнее пошагового (0.5): при 50% война окупалась наполовину и
  // не сходилась; 0.36 = 0.3 + 30% срезанного (тот же компромисс)
  WRECK_LOOT_FRAC: 0.36,
...
export const RT_OUTPOST_MS = 12000;     // реалтайм: период дохода/ремонта/выстрела форта (раунд-темп +30%, см. RT.INCOME_MS)

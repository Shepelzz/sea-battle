...
// Клетка «листка в клетку» — базовая единица расстояний для ЧЕЛОВЕКА (и для 🧠 ИИ-соперника:
// он видит карту в клетках, а не в игровых единицах — см. server/ai/perception.js).
export const CELL = 40;
// 👁 Туман войны: радиусы обзора. ЗЕРКАЛО клиентских FOG_SHIP_MULT/FOG_BASE_EXTRA в
// public/js/game.js (клиент не импортирует серверный конфиг) — меняешь тут, поменяй и там.
// ИИ-соперник смотрит ровно этими же глазами, иначе он не соперник, а читер.
export const FOG_SHIP_MULT = 1.3;   // радиус обзора корабля = max(ход, дальность огня) × это
export const FOG_BASE_EXTRA = 200;  // радиус обзора базы = её радиус + это

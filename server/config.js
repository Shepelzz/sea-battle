...
// Сейчас: урон всего стреляющего ×3 (корабль тонет за 2–3 попадания), ответка порта 20 и НЕ
// растёт с уроном (иначе одинокий фрегат умирал за выстрел до победы, и осады срывались раз за
// разом — хвост партий уходил за 100). Порт оставлен 840 и карта полная — решения владельца:
// два тычка линкором — это раш, а не осада, а на карте 1000×750 негде развернуться. База держит
// 7 мортир фрегата или 3 линкора. Бот-против-бота, Адмирал: медиана ~110 раундов (было ~215). Лечение (ремонтник, док, фактория) ×2 —
...
// ─── Порт / база ──────────────────────────────────────────────────────────────
export const PORT_HP = tune('PORT_HP', 840);   // осада — дело долгое: 7 мортир фрегата или 3 линкора
export const PORT_RETURN_DMG = tune('PORT_RETURN_DMG', dmg(20)); // порт огрызается по атакующему кораблю
...
export const BATTERY_RETURN_DMG = dmg(32);    // ответка порта с батареей (обычная — PORT_RETURN_DMG)
...
export const OUTPOST_LEVELS = [
  ...
  { icon: '🏪', price: 250, hp: 180, income: 8, heal: tune('OUTPOST_HEAL', 0.06) },
  { icon: '🏰', price: 400, hp: 260, income: 12, heal: tune('OUTPOST_HEAL', 0.06), gun: dmg(45) },
];

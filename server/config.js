...
// ⛵ Строй идёт по самому медленному, но на четверть ДАЛЬШЕ его одиночного хода: кильватер
// тянет. Это награда за то, что флот ведут вместе, а не гонят корабли поодиночке, — и способ
// сократить долгую дорогу до врага на большой карте (карту решили не ужимать). Без потолка
// нарочно: три шхуны строем идут 212, а не 170, — однородный флот и есть главный случай.
// Эксплойта «пришёл и выстрелил» нет: строй стоит манёвр и помечает все суда сходившими.
export const CONVOY_RANGE_MULT = 1.25;
export const convoyMoveRange = (types) => Math.min(...types.map(t => SHIP_TYPES[t].move)) * CONVOY_RANGE_MULT;
...
// ⚠ ОТЛАДКА РАСКРЫВАЕТ ЧУЖИЕ ФИНАНСЫ. Поэтому на проде её включить НЕЛЬЗЯ, даже поставив
// SB_DEBUG=1: одна забытая переменная окружения сделала бы казну всех игроков публичной.
// Прод узнаём по NODE_ENV=production или по настроенной внешней базе (локально её нет).
// Хочешь смотреть решения бота на боевом сервере — поднимай копию без DATABASE_URL.
export const PRODUCTION = process.env.NODE_ENV === 'production' || !!(
  process.env.DATABASE_URL || process.env.MYSQL_URL || process.env.JAWSDB_URL ||
  process.env.CLEARDB_DATABASE_URL || process.env.DB_HOST || process.env.MYSQLHOST);
export const DEBUG_REQUESTED = /^(1|true|yes|on)$/i.test(process.env.SB_DEBUG || '');
export const DEBUG = DEBUG_REQUESTED && !PRODUCTION;

// ─── 🎛 Подбор баланса ───────────────────────────────────────────────────────
// SB_TUNE='{"DMG_MULT":2,"PORT_HP":400}' перекрывает константы, помеченные tune() ниже.
// Нужно симулятору (tools/sim.mjs, tools/tune.mjs): гонять сотни партий с разными цифрами,
// не правя файл между прогонами. На проде переменная игнорируется — баланс боевого сервера
// живёт только в этом файле, а не в окружении.
const TUNE = (() => {
  if (PRODUCTION) return {};
  try { return JSON.parse(process.env.SB_TUNE || '{}'); } catch { return {}; }
})();
const tune = (key, def) => (key in TUNE ? TUNE[key] : def);
// ─── Темп партии (подобрано симулятором, tools/tune.mjs) ─────────────────────
// Исходные цифры (урон 15/28/42/65, порт 840, карта 1600×1200) давали войну на истощение:
// первый выстрел на ~20-м раунде, фрегат тонул за 8–9 обменов, партия двоих — ~250 раундов.
// Сейчас: урон всего стреляющего ×3 (корабль тонет за 2–3 попадания), ответка порта тоже ×3 (60):
// одиночный линкор гибнет на 4-м из 6 залпов по порту, осада — дело пары-тройки кораблей.
// Карта полная, порт 1680 и аванпосты ×2 — решения владельца: на карте 1000×750 негде
// развернуться, а укрепление за пару тычков — это раш, не осада. Порт держит 14 мортир фрегата
// или 6 линкора (до всего этого — 20 и 9). Симулятор звал к порту 300 и ответке 20 (партия
// короче на десяток раундов) — но темп меряли боты, а чувствовать будут люди. Лечение (ремонтник, док, фактория) ×2 —
// чтобы починка по-прежнему снимала примерно одно попадание.
//
// Общий множитель урона ВСЕГО, что стреляет: корабли, пираты, ответка порта, пушка форта.
// Меняет темп боя, не трогая соотношения между классами. Прочность судов не трогаем —
// от неё зависят ремонт и лечение (доли от максимума), они масштабируются сами.
export const DMG_MULT = tune('DMG_MULT', 1);
const dmg = base => Math.round(base * DMG_MULT);
// Ход всех судов (и пиратов) — темп сближения: сколько раундов флоты идут навстречу.
export const MOVE_MULT = tune('MOVE_MULT', 1);
const mv = base => Math.round(base * MOVE_MULT);

// ─── Ограничения против злоупотреблений ──────────────────────────────────────
// Партия — это полный JSON состояния в памяти и в базе. Без потолка скрипт в цикле
// набивает и то и другое за минуты, поэтому столько НЕзавершённых партий игрок может
// СОЗДАТЬ (лобби считаются тоже). Партии, куда его позвали гостем, в счёт не идут:
// чужое лобби ресурсов ему не стоит, и наказывать за приглашения незачем.
export const MAX_ACTIVE_GAMES = 3;
// 📯 «Поторопить» шлёт настоящее письмо. Уйдёт оно, только если получатель согласен
// (чекбокс в профиле, колонка players.mail_nudge, по умолчанию включён) И ему давно не писали:
// у КАЖДОГО адресата своя пауза, сколько бы партий он ни вёл. Согласие тут главное, пауза —
// страховка от потока. Механика хода работает всегда, режется только письмо.
export const NUDGE_MAIL_COOLDOWN_MS = 30 * 60 * 1000;
// 🏆 Порог «партия состоялась». Без него рейтинг фармился белым флагом: победителю 3 очка,
// сдавшемуся 1 (второе место), и всё это за полминуты — в лидерборде такая победа видна как
// строка «1 игра, 1 победа, урон 0, потоплено 0». Очки даём только за бой, который правда был.
//
// Минимум для ЛЮБОЙ партии: столько раундов и столько урона между ЛЮДЬМИ (бой по НПС не в счёт).
export const RANKED_MIN_ROUNDS = 4;
export const RANKED_MIN_DAMAGE = 1;
// Если партию решила СДАЧА, требуем больше: одними раундами тут не отделаешься — их
// накликивают пропуском хода. Сдача считается настоящей капитуляцией, только когда
// сдавшийся и правда разбит: людьми потоплено столько кораблей И у сдавшегося в казне
// меньше, чем стоит самый дешёвый боевой корабль (бриг) — отстроиться ему уже не на что.
export const RANKED_MIN_SUNK_ON_QUIT = 4;
export const RANKED_BROKE_SHIP = 'brig';
// Победу осадой размен флотами не сопровождает: можно вынести порт, не потопив ни одного корабля.
// Поэтому у «разбит» есть второй признак — порт сдавшегося разрушен больше чем на столько.
// Подделать его так же дорого: порт огрызается по атакующему, это минуты настоящей игры.
export const RANKED_PORT_WRECK = 0.5;
// ─── Игровые режимы ───────────────────────────────────────────────────────────
...
export const REPAIR_HEAL_FRAC = tune('REPAIR_HEAL_FRAC', 0.3);
...
export const SHIP_TYPES = {
  barkas: {
    icon: '⛵', price: 60,
    hp: 30, dmg: dmg(15), fireRange: 70, move: mv(180),
    fishing: FISH_INCOME // золота КАЖДЫЙ ХОД, пока стоит в рыбном месте (без траты действия)
  },
  shkhuna: {
    icon: '🛥', price: 110,
    hp: 60, dmg: dmg(45), fireRange: 110, move: mv(170), fishing: 0 // dmg = урон залпа в упор по перпендикуляру (макс)
  },
  brig: {
    icon: '⚓', price: 220,
    hp: 110, dmg: dmg(84), fireRange: 140, move: mv(135), fishing: 0
  },
  fregat: {
    icon: '🚢', price: 380,
    hp: 170, dmg: dmg(126), fireRange: 165, move: mv(110), fishing: 0 // dmg = база урона: залп в упор по перпендикуляру И база мортиры (по судам ×0.5)
  },
  linkor: {
    icon: '🛳', price: 500,
    hp: 280, dmg: dmg(195), fireRange: 190, move: mv(90), fishing: 0, // dmg = база: залп в упор по перпендикуляру (65) И база мортиры
    portBonus: 1.5 // урон МОРТИРЫ по вражескому порту в 1.5 раза — главный осадный корабль (65×1.5≈98)
  },
  repair: {
    icon: '🛟', price: 280,
    hp: 130, dmg: 0, fireRange: 115, move: mv(135), fishing: 0,
  ...
  carrier: {
    ...
    hp: 1400, dmg: dmg(195), fireRange: 320, move: mv(280), fishing: 0, volley: 5
  }
};

// ─── Старт ──────────────────────────────────────────────────────────────────
export const START_FLEET = ['shkhuna', 'shkhuna', 'fregat']; // пара шхун и фрегат
export const START_GOLD = tune('START_GOLD', 350);
...
// ─── Порт / база ──────────────────────────────────────────────────────────────
export const PORT_HP = tune('PORT_HP', 1680);  // урон ×3 → порт ×2: 14 мортир фрегата или 6 линкора (было 20 и 9)
export const PORT_RETURN_DMG = tune('PORT_RETURN_DMG', dmg(60)); // порт огрызается по атакующему кораблю: ×3, как урон флота
export const PORT_RETURN_LINKOR_MULT = 1.2; // …линкору — на 20% больнее
export const PORT_INCOME = tune('PORT_INCOME', 6); // доход порта в начале хода игрока
...
export const BATTERY_RETURN_DMG = dmg(96);    // ответка порта с батареей (обычная — PORT_RETURN_DMG)
...
export const DRYDOCK_HEAL = tune('DRYDOCK_HEAL', 0.1);        // …и доля МАКСИМАЛЬНОГО HP за тик
...
export const OUTPOST_LEVELS = [
  { icon: '⛺', price: 150, hp: 240, income: 3 },
  { icon: '🏪', price: 250, hp: 360, income: 8, heal: tune('OUTPOST_HEAL', 0.06) },
  { icon: '🏰', price: 400, hp: 520, income: 12, heal: tune('OUTPOST_HEAL', 0.06), gun: dmg(45) },
];
...
export const WRECK_LOOT_FRAC = tune('WRECK_LOOT_FRAC', 0.5);     // лут с обломков потопленного корабля (доля цены)
...
export const PIRATE = {
  ...
  // урон ×2, а не ×3 как у флота: пират — помеха, не армия. Баркас (30 HP) переживает один
  // выстрел — у рыбака остаётся ход, чтобы уйти; шхуна держит два.
  hp: 80, dmg: dmg(24), fireRange: 130, move: mv(80), fishing: 0
};
...
// ПАУЗА ПЕРЕД ПОПОЛНЕНИЕМ. Раньше убыль восполнялась мгновенно: потопил пирата — в тот же ход
// «на горизонте появился пиратский корабль». Для того, кто на них охотится, это выглядело так,
// будто пираты валят непрерывно. Замер показал, что растворение в тумане тут почти ни при чём
// (0.08 появления на сто ходов), а весь поток шёл именно из мгновенного доспавна после убийств.
// Теперь новый приходит не раньше, чем через столько ходов после того, как место освободилось.
// Подобрано замером «охотника» (топим пирата сразу, как появится): без паузы — 8.7 появлений
// на сто ходов, с паузой в 3 хода — 3.7, то есть примерно вдвое реже, как и просили.
export const PIRATE_RESPAWN_DELAY = 3;      // пошагово — в ходах
export const RT_PIRATE_RESPAWN_MS = 20000;  // реалтайм — то же вдвое реже (было 10 с)
...
// ─── Масштаб карты по числу игроков (по умолчанию 1 — полная карта всем) ─────
// Ручка для симулятора: SB_TUNE='{"MAP_SCALE":{"2":0.625}}' ужимает карту двоих. Замер показал,
// что ближе базы — раньше первый выстрел (9-й раунд вместо ~20-го) и партия короче на треть,
// но на 1000×750 негде развернуться (решение владельца — карту не трогать). Если масштаб
// включать, вместе с картой масштабируются ЗАЗОРЫ РАСКЛАДКИ (отступ баз от углов, промежутки
// между островами и рыбой, отступ спавна пиратов), а размеры баз, островов и рыбных зон — нет:
// иначе на малой карте острова не помещались (четверым генератор не ставил ни одного).
export const MAP_SCALE_BY_PLAYERS = tune('MAP_SCALE', {});
export const mapScaleFor = n => MAP_SCALE_BY_PLAYERS[n] ?? MAP_SCALE_BY_PLAYERS[4] ?? 1;
